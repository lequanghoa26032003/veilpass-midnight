/**
 * CLI for interacting with test-app contract
 */
...
import { createInitialPrivateState, witnesses } from './witnesses';
...
// the same encrypted credential secret.
...
const compiledContract = CompiledContract.make('veilpass', VeilPass.Contract).pipe(
  // The generated module is loaded dynamically after the existence check, so
  // TypeScript cannot infer its witness type even though the runtime validates it.
...
async function main() {
  ...
  try {
    ...
    while (running) {
      console.log('─── Menu ───────────────────────────────────────────────────────');
      console.log('  1. Claim a benefit');
      console.log('  2. Read public claim statistics');
      ...
      switch (choice.trim()) {
        case '1': {
          const campaign = await rl.question('  Enter campaign name or ID: ');
          const campaignId = createHash('sha256').update(campaign).digest();
          console.log('\n  Submitting transaction (this may take 30-60 seconds)...');
          try {
            const tx = await deployed.callTx.claimBenefit(campaignId);
            console.log(`\n  ✅ Benefit claimed for campaign: "${campaign}"`);
        ...
        case '2': {
          ...
          try {
            ...
            if (contractState) {
              const ledgerState = VeilPass.ledger(contractState.data);
              console.log(`\n  📋 Total successful claims: ${ledgerState.totalClaims}`);
              console.log(`  Published nullifiers: ${ledgerState.usedNullifiers.size()}\n`);
            } else {
              console.log('\n  📋 Contract state is not indexed yet\n');
            }
          } catch (error) {
            console.error('\n  ❌ Failed:', error instanceof Error ? error.message : error);
          }
